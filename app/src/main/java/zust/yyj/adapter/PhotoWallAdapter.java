...
public class PhotoWallAdapter extends ArrayAdapter<String> implements AbsListView.OnScrollListener {
    ...
    /**
     * 图片缓存技术的核心类，用户缓存所有下载好的图片，在程序内存达到设定值时会将最少最近使用的图片移除掉
     */
    ...
    public void initWithUrls(Images imgs){
        this.images = imgs;
    }
    @Override
    ...
    private void loadBitmaps(int firstVisibleItem, int visibleItemCount) {
        try {
            for (int i = firstVisibleItem; i < firstVisibleItem + visibleItemCount; i++) {
                String imageUrl = images.getImageThumbUrls().get(i);
                Bitmap bitmap = getBitmapFromMemoryCache(imageUrl);
                if (bitmap == null) {
    ...
    class BitmapWorkerTask extends AsyncTask<String,Void,Bitmap>{
        ...
        protected void onPostExecute(Bitmap bitmap) {
            ...
            //根据Tag找到相应的ImageView空间，将下载好的图片显示出来
